...
bool encoderVolumeMode = true; // Encoder controls volume by default (true=volume, false=frequency)
...
void loop()
{
  ...
  else
  {
      ...
      switch(currentCmd)
      {
        case CMD_NONE:
          if (encoderVolumeMode) {
            ...
            // Encoder controls frequency
            needRedraw |= doTune(encCountAccel);
            // Frequency changed; save band settings
            prefsRequestSave(SAVE_CUR_BAND);
          }
          break;
        case CMD_SCAN:
    ...
    else if(pb1st.wasClicked || pb1st.wasShortPressed)
    {
      ...
      {
        // Toggle encoder control mode between volume and frequency
